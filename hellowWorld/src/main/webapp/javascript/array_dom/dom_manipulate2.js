...
let trs = /* document.getElementsByTagName('tbody')[0].children; */
 document.querySelectorAll('tbody>tr'); //tbody의 tr요소를 다 가 져 오겠다. | :nth~하면 순번까지 가져올수있음
console.log(trs);
...
trs.forEach(tr => {
    ...
    if (tr.children[1].textContent == id) {
        tr.setAttribute('class', 'focus');
        // 값변경
        let childTd = tr.children;
        childTd[2].textContent = document.getElementById('mname').value; //name
        childTd[3].textContent = document.getElementById('mphone').value; //name
    }
});
}
...
tr.addEventListener('click', function () {
    ...
    document.getElementById('mid').value = this.children[1].innerText; //id
    document.getElementById('mname').value = this.children[2].innerText; //name
    document.getElementById('mphone').value = this.children[3].innerText; //phone
}, false);
